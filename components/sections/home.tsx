    ...
    return (
        <div ref={ref} className="h-screen text-white relative px-6 sm:px-10">
            <div className="absolute inset-0 flex justify-center items-center z-20">
                <div className={`w-full ${isMobile ? 'max-w-full' : 'max-w-[1500px]'} text-center`}>
                    <div className="block">
                        <motion.h1 
                                ...
                                <div key={lineIndex} className="block">
                                    {line.split("").map((char, charIndex) => (
                                        <motion.span
                                            ...
                                            transition={{
                                                delay: (lineIndex * line.length + charIndex) * 0.05 + 0.1,
                                                duration: 0.6,
                                                ease: [0.25, 0.46, 0.45, 0.94]
                                            }}
                                            className="inline-block"
                                            style={{ 
                                                marginRight: char === ' ' ? (isMobile ? '0.15em' : '0.2em') : '0'
                                            }}
                                        >
                                            {char === ' ' ? '\u00A0' : char}
                                        </motion.span>
                    ...
                    <motion.div 
                        className={`mx-auto ${isMobile ? 'mt-3 px-2' : 'mt-4 sm:mt-6'}`}
                    ...
                    {/* Social Links */}
                    <motion.div 
                        className={`w-full ${isMobile ? 'mt-6' : 'mt-8 sm:mt-12'}`}
                        initial={{ opacity: 0 }}
                        animate={inView ? { opacity: 1 } : { opacity: 0 }}
                        transition={{ delay: 1.0, duration: 0.8 }}
                    >
                        <div className={`flex justify-center items-center w-full ${isMobile ? 'space-x-4' : 'space-x-6 sm:space-x-8'}`}>
                            {socials.map((social, index) => (
                                ...
                                >
                                    ...
                                    >
                                        <social.icon className={`text-white group-hover:text-white/90 transition-colors duration-300 ${isMobile ? 'text-lg' : 'text-xl sm:text-2xl'}`} />
                                    </motion.div>
                        ...
                        <div className="flex justify-center w-full mt-4">
                            <motion.div
                                className={`h-px bg-linear-to-r from-transparent via-purple-400/60 to-transparent ${isMobile ? 'w-48' : 'w-64'}`}
                                initial={{ scaleX: 0 }}
                    ...
                    {!isMobile && (
                        ...
                        >
                            <motion.button
                                onClick={() => {
                                    const nextSection = document.getElementById('experience') || document.getElementById('about');
                                    if (nextSection) {
